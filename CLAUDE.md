...
### What's next
- **Wire `EligibilityProfile` into the BM25 filter dict** (or as a post-RRF gate) — single highest-leverage open task, now CONFIRMED-MEASURED by Phase C4 expanded eval (both v1 0.540 and v2 0.526 well below the 0.68 complex gate; more data won't fix this). Three of the five worst-NDCG queries fail for the same reason: the agent parses age / prior treatments / progression status into `PatientProfile`, but the orchestrator does not translate those into hard retrieval filters. Q410 (medulloblastoma 6 yo) loses to ADULT and post-pubertal trials; Q413 (failed osimertinib) retrieves osimertinib-naive trials at rank 5; Q416 (post-trastuzumab progression) ranks the STOP-HER2 trial at #1 (the *opposite* clinical situation). Lifting these from NDCG@10 ≈ 0.42–0.55 toward 0.85+ requires no retraining.
- **Re-run Phase C4 after the eligibility-filter fix** — same 80-query labeled set (or just the 15 complex + 15 vague slices, ~$0.30) to measure whether wiring `EligibilityProfile` actually lifts complex from 0.526 → above the 0.68 gate. This is the apples-to-apples test the gate was originally designed for.
