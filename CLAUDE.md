...
## Current State
Last updated: 2026-05-13 (Week 10 / Phase C close-out)
...
- **Parser quality: UMLS via SciSpacy `EntityLinker`** (Decision 18 upgrade path) — now the top open task. Phase C5a measured the eligibility filter at +0.075 to +0.101 NDCG@5 on the 4 queries where it triggers, but it only triggers on 13 % of complex queries because `required_prior_treatments` uses literal string match (misses `osimertinib` → `EGFR TKI` drug-class equivalences). Adding UMLS-backed drug-class matching would let the filter catch Q413 (`failed osimertinib` → drop osimertinib-naive trials) and Q416 (`post-trastuzumab progression` → drop trastuzumab-required-naive trials), which are the canonical complex failures. Once parser precision on prior treatments crosses ~90 %, `HARD_FILTER_CRITERIA` can be extended in `eligibility_filter.py` to include `required_prior_treatments`.
- **Better patient-profile extraction on caregiver phrasings** — the filter fires on 0 of 15 vague queries because `QueryParserAgent` (Haiku) doesn't extract `age` or `sex` from queries like "my dad has cancer". A short prompt tweak ("if relationship language implies adult/elderly patient, infer age=60") might unlock filtering on vague queries — but be careful about confidently-wrong outputs (Decision 19's "Unknown over false Met").
